    ...
    <div className="app-container">
      ...
      {showNew && (
        <div className="card mb-4 animate-fade-in" style={{ background: "var(--card-muted)" }}>
          ...
          <div className="flex gap-2"><input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Drink water, stretch..." className="input flex-1" autoFocus /><button onClick={addHabit} disabled={!newName.trim()} className="btn btn-primary" style={{ opacity: newName.trim() ? 1 : 0.5 }}>Add</button></div>
        </div>
      )}
