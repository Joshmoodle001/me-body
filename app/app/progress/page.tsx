          ...
          <div className="dialog-content">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="metricWeight" className="input-label">Weight (kg)</label><input id="metricWeight" type="text" inputMode="decimal" value={weight} onChange={(e)=>{let v=e.target.value.replace(/[^0-9.]/g,"");const p=v.split(".");if(p.length>2)v=p[0]+"."+p.slice(1).join("");setWeight(v)}} placeholder={latest?.weightKg?.toString() ?? ""} className="input" /></div>
                <div><label htmlFor="metricWaist" className="input-label">Waist (cm)</label><input id="metricWaist" type="text" inputMode="decimal" value={waist} onChange={(e)=>{let v=e.target.value.replace(/[^0-9.]/g,"");const p=v.split(".");if(p.length>2)v=p[0]+"."+p.slice(1).join("");setWaist(v)}} placeholder={latest?.waistCm?.toString() ?? ""} className="input" /></div>
                <div><label htmlFor="metricSleep" className="input-label">Sleep (hrs)</label><input id="metricSleep" type="text" inputMode="decimal" value={sleep} onChange={(e)=>{let v=e.target.value.replace(/[^0-9.]/g,"");const p=v.split(".");if(p.length>2)v=p[0]+"."+p.slice(1).join("");setSleep(v)}} placeholder={latest?.sleepHours?.toString() ?? ""} className="input" /></div>
                <div><label htmlFor="metricSteps" className="input-label">Steps</label><input id="metricSteps" type="text" inputMode="numeric" value={steps} onChange={(e)=>setSteps(e.target.value.replace(/\D/g,""))} placeholder={latest?.steps?.toString() ?? ""} className="input" /></div>
              </div>
