...
export default function WaterPage() {
  ...
  const load = async () => { const p = await getProfile(); setProfile(p ?? null); if (p) { const t = await getTargets(p.id); setTargets(t ?? null); } const wLogs = await getWaterLogsForDate(todayStr()); setLogs(wLogs); setLoading(false); };
  useEffect(() => { load(); }, []);
    ...
    <div className="app-container">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {[250, 350, 500, 750].map((ml) => (
          <button key={ml} onClick={() => addWater(ml)} className="card text-center py-3.5 font-semibold text-sm transition-all" style={{ color: "var(--water)", borderColor: "var(--border)" }}>+{ml}ml</button>
        ))}
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); const ml = Number(customMl); if (ml > 0 && ml <= 5000) { addWater(ml); setCustomMl(""); } }} className="flex gap-2 mb-6">
        ...
        <button type="submit" disabled={!customMl} className="btn btn-primary" style={{ opacity: customMl ? 1 : 0.5, background: "var(--water)", boxShadow: "0 4px 16px rgba(108,215,255,0.25)" }}>Add</button>
      </form>

      {logs.length > 0 && (
        <div><p className="text-xs font-semibold uppercase mb-2" style={{ color: "var(--text-muted)", letterSpacing: "0.05em" }}>Today&apos;s logs</p>
          <div className="space-y-2">
            {logs.map((log) => (
              <div key={log.id} className="flex items-center justify-between card px-4 py-3">
                <div className="flex items-center gap-3"><span className="text-base">&#x1F4A7;</span><span className="font-semibold text-sm tabular-nums" style={{ color: "var(--text-primary)" }}>{log.amountMl}ml</span></div>
                <div className="flex items-center gap-2"><span style={{ fontSize: "11px", color: "var(--text-muted)" }}>{new Date(log.loggedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                  <button onClick={async () => { await deleteWaterLog(log.id); load(); }} className="p-1 rounded-lg transition-colors" style={{ color: "var(--text-muted)" }} aria-label="Delete"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div>
              </div>
            ))}
          </div>
        </div>
