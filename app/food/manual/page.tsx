  ...
  return (
    <div className="app-container">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
        <div><label htmlFor="fname" className="input-label">Name *</label><input id="fname" type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Homemade chicken salad" className="input" /></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label htmlFor="fbrand" className="input-label">Brand</label><input id="fbrand" type="text" value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="Optional" className="input" /></div>
          <div><label htmlFor="fbarcode" className="input-label">Barcode</label><input id="fbarcode" type="text" value={barcode} onChange={(e) => setBarcode(e.target.value)} placeholder="Optional" className="input" /></div>
        </div>
        <div><label htmlFor="fserving" className="input-label">Serving Size (g)</label><input id="fserving" type="text" inputMode="numeric" value={servingSizeG} onChange={(e) => setServingSizeG(e.target.value.replace(/\D/g,""))} className="input" /></div>
