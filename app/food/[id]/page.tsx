    ...
    <div className="app-container">
      ...
      <div className="card mb-4">
        ...
        <div className="space-y-3">
          <div><label htmlFor="quantity" className="input-label">Quantity (g)</label><input id="quantity" type="text" inputMode="numeric" value={quantity} onChange={(e) => setQuantity(e.target.value.replace(/\D/g,""))} className="input" /></div>
          <div><label htmlFor="mealType" className="input-label">Meal</label><select id="mealType" value={mealType} onChange={(e) => setMealType(e.target.value as MealType)} className="input">{MEAL_TYPES.map((m) => (<option key={m} value={m} className="capitalize">{m}</option>))}</select></div>
